...
pub(crate) fn with_progress<F, T>(f: F, n_iter: u64, pb_msg: &'static str, min_t: Duration) -> T
where
    ...
    T: Send,
{
    ...
    thread::scope(|s| {
        ...
        let pb_thread_handle = s.spawn(move || {
            ...
            loop {
                ...
                thread::park_timeout(min_t - elapsed);
            ...
            let pb = ProgressBar::new(n_iter)
                .with_style(
                    ProgressStyle::default_spinner().template(
                        "{prefix}{msg}: {percent:02}%|{wide_bar}| {pos}/{len} [{elapsed}<{eta}, {per_sec}{postfix}]"
                    ).unwrap()
